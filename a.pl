...
die "Config '$config' must contain 'profiles' as a list\n"
    unless ref($cfg) eq 'HASH' && ref($cfg->{profiles}) eq 'ARRAY';
...
die "No profile matched input '$base' in config '$config'\n" unless $profile;
...
while (1) {
    ...
    $csv->parse($line) or do {
        ...
        die "CSV parse error at input offset " . ($pos - $base_offset) . ": $err\nLine: $line\n";
    };
    my @f = $csv->fields();
